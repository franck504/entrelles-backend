...
const chatWithAi = async (req, res) => {
    try {
        ...
        // Si pas de clé, on simule une réponse pour le développement
        if (!apiKey || apiKey === 'YOUR_GEMINI_API_KEY') {
            return res.status(200).json({
                success: true,
                reply: `[MODE DEV - Simulé] Bonjour ! Je suis l'assistante Entrelles. Concernant votre question : "${message}", sachez qu'Entrelles est une plateforme de covoiturage 100% féminine. Pour utiliser pleinement mes capacités de réponse, veuillez configurer une clé API Gemini valide.`
            });
        }
...
Réponds de manière chaleureuse, bienveillante et professionnelle. Utilise le féminin. Si une question ne concerne pas Entrelles, ramène poliment la conversation sur le service.`;
